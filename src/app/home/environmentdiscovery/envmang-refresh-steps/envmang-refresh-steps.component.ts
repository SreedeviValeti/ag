...
import { LOADCACHESTEPS, OSs, REFRESHSTEPS, stepObj } from 'src/app/shared/constants/adminstration-constants';
import { StepModelComponent } from '../../pillar-config/edit-administration/steps-edit/step-model/step-model.component';
...
@Component({
    selector: 'envmang-refresh-steps',
    templateUrl: './envmang-refresh-steps.component.html',
    styleUrls: ['./envmang-refresh-steps.component.scss']
})

export class EnvmangRefreshStepsComponent implements OnChanges {
    ...
    onFileChange(event) {
      ...
      if(event.target.files && event.target.files.length > 0) {
        ...
        reader.onload = () => {
      //testing to do
      const fileurl = `https://${environment.fileuploaddomain}/${environment.saas_bucket}/client-scripts/${environment.account}/${environment.region}/powershell-scripts/${localStorage.getItem("envname")}/${file.name}`
      console.log(fileurl)
      // this.infraService.fileUpload(fileurl,file).subscribe(
      //   data => {
      //     console.log(data)
      //   },
      //   err => {
      ...
      // this.http.put('https://e2twy6ntab.execute-api.us-east-2.amazonaws.com/s3/agsaasohio-artifacts-20231024170431344300000001/client-scripts/729444515875/us-east-2/shell-scripts/cssnd/test.pdf', file
    ...
    loadStepsFromFile(filedata) {
      let payload = {"envname" :"cstrn"}

      // const activityProp = REFRESHSTEPS
      ...
      // this.http.put('https://api-agsaasohio-s3.active-genie.com/agsaasohio-artifacts-20231024170431344300000001/client-scripts/729444515875/us-east-2/shell-scripts/cssnd/test.pdf', filedata
    ...
    validateSteps(doSubmit: boolean) {
      ...
      this.steps.forEach(step => {
        ...
        let payload = {"envname" :"cstrn"}
        observables.push(this.envService.envmangementrefresh(this.refreshurl,payload));
      });
